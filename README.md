<!--
  GitHub Profile README for Priyanshu Kesharwani
  Replace YOUR_GITHUB_USERNAME, your social links and any placeholders below.
-->

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?color=00A3FF&width=680&height=40&lines=Hi+there+%F0%9F%91%8B+I'm+Priyanshu+Kesharwani.;Full-Stack+Developer+%7C+MERN+%7C+Java+%7C+DSA" alt="Typing SVG" />
</p>

<p align="center">
  <a href="mailto:priyanshukesharwani885857@gmail.com"><img alt="email" src="https://img.shields.io/badge/Email-priyanshukesharwani885857@gmail.com-%23D14836?style=for-the-badge&logo=gmail&logoColor=white"></a>
  <a href="tel:+918858576463"><img alt="phone" src="https://img.shields.io/badge/Phone-+91%208858576463-1A73E8?style=for-the-badge&logo=telephone"></a>
  <a href="https://www.linkedin.com/in/YOUR_LINKEDIN"><img alt="linkedin" src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin"></a>
</p>

---

## 👨‍💻 About Me
I’m a **Computer Science Engineering** graduate from **Technocrats Institute of Technology, Bhopal** (CGPA: 7.9).  
I build production-ready web apps and polished UI experiences — from component libraries to end-to-end MERN applications. I focus on clean code, reusability, and great UX.

**Core strengths:** Full-Stack (MERN), Java, DSA, UI/Frontend, System design, Clean & scalable code.

---

## 🛠 Tech Stack & Tools
<p>
  <!-- Icons: keep only those relevant — replace/add as needed -->
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original-wordmark.svg" alt="HTML5" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original-wordmark.svg" alt="CSS3" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="JS" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original-wordmark.svg" alt="React" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original-wordmark.svg" alt="Node.js" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/express/express-original-wordmark.svg" alt="Express" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg" alt="MongoDB" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" alt="Java" width="36" height="36"/>
  <img src="https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg" alt="Firebase" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-plain.svg" alt="Git" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/bootstrap/bootstrap-plain.svg" alt="Bootstrap" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/vscode/vscode-original.svg" alt="VSCode" width="36" height="36"/>
  <img src="https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg" alt="tailwind" width="36" height="36"/>
  <img src="https://www.vectorlogo.zone/logos/google_cloud/google_cloud-icon.svg" alt="gcp" width="36" height="36"/>
</p>

---

## 💼 Experience
**Coding Thinker — Full Stack Development (Intern)**  
- Contributed to end-to-end features, wrote modular frontends and backend endpoints.  
- Debugged and optimized performance bottlenecks.  
- Followed best practices for maintainable and scalable code.

---

## 🏆 Competitive Programming & Achievements
- **LeetCode Weekly Contest 444:** Rank **3367 / 28,833**  
- **Coding Thinker 45-Day Challenge:** Rank **9 / 350**  
- Google & IBM certifications in Project Management and Digital Strategy (as noted in resume).

---

## 🎓 Education
**Technocrats Institute of Technology — B.Tech (CSE)**  
Bhopal, Madhya Pradesh — CGPA: **7.9**

---

## 📫 Contact & Connect
- **Phone:** +91 8858576463  
- **Email:** priyanshukesharwani885857@gmail.com  
- **LinkedIn:** https://www.linkedin.com/in/priyanshu-kesharwani-57a525255/
- **LeetCode:** https://leetcode.com/u/OutR5t3bfn/
- **Portfolio:** https://priyanshu-s-portfolio-six.vercel.app/

---

## ✨ What I'm open to
- Freelance or full-time roles in Frontend / Full-Stack development.  
- Collaboration on UI libraries, open-source component systems, and performance optimization projects.  
- Mentoring juniors on frontend fundamentals, React, and problem solving.

---

## Fun little things
- I enjoy turning UI ideas into reusable components and writing maintainable code.
- Believer in shipping small improvements frequently.
